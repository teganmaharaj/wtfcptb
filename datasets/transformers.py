...
class InputDropper(Transformer):
    """Randomly drop input on a datastream.
    """
    def __init__(self, datastream, rng):
        super(InputDropper, self).__init__(datastream)
        self.axes_list = axes_list
        self.produces_examples = False

    def sample_mask(self, prob, rng):
        mask = rng.normal(0.0, 1.0, (dim, dim))
        return mask
...
class AddUniformAlignmentMask(Transformer):
    ...
    def get_data(self, request=None):
        ...
        for k in xrange(batch_size):
            ...
            # DEBUG
            #plt.figure()
            #plt.imshow(alignment[:,k,:], cmap='gray', interpolation='none')111df
            #plt.show()
        data = data + (alignment,)
